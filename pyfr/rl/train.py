...
# TorchRL >= 0.13 uses MultiSyncCollector; keep fallback for older releases.
try:
    from torchrl.collectors import MultiSyncCollector
except ImportError:  # pragma: no cover - compatibility path
    from torchrl.collectors import MultiSyncDataCollector as MultiSyncCollector
...
class _SingleEnvCollector:
    ...
    def __iter__(self):
        for _ in range(self._nbatches):
            # Match reset_at_each_iter=True behaviour used by MultiSyncCollector.
            self.env.reset()
            with torch.no_grad():
    ...
    def update_policy_weights_(self):
        # Policy object is shared in-process; no sync required.
        return None
...
def train_agent(mesh_file, cfg_file, backend_name, checkpoint_dir='checkpoints', ic_dir=None, load_model=None):
    ...
    print(f'Using MultiSyncCollector with {num_devices} environment(s).')
