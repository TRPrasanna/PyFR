...
<%pyfr:macro name='bc_rsolve_state' params='ul, nl, ur' externs='ploc, t, control_params, t_act_interval, actuator_bounds, z_slot_bounds'>
% for v in 'uvw'[:ndims]:
    fpdtype_t bc_${v} = 0.0;
% endfor

% for aidx in range(nctrl):
    fpdtype_t control_${aidx} = (control_params[${aidx}][1] - control_params[${aidx}][0])
                              / t_act_interval[0][0]
                              * (t - control_params[${aidx}][2])
                              + control_params[${aidx}][0];
    fpdtype_t lower_bound_${aidx} = min(control_params[${aidx}][0],
                                        control_params[${aidx}][1]);
    fpdtype_t upper_bound_${aidx} = max(control_params[${aidx}][0],
                                        control_params[${aidx}][1]);
    control_${aidx} = max(lower_bound_${aidx},
                          min(control_${aidx}, upper_bound_${aidx}));
    fpdtype_t mask_${aidx} = 1.0;
% for didx in range(ndims):
    mask_${aidx} *= (ploc[${didx}] >= actuator_bounds[${aidx}][${2*didx}]
                      && ploc[${didx}] <= actuator_bounds[${aidx}][${2*didx + 1}])
                     ? 1.0 : 0.0;
% endfor
% if ndims == 3 and nzslots:
    fpdtype_t z_slot_mask_${aidx} = 0.0;
% for sidx in range(nzslots):
    z_slot_mask_${aidx} += (ploc[2] >= z_slot_bounds[${sidx}][0]
                             && ploc[2] <= z_slot_bounds[${sidx}][1])
                            ? 1.0 : 0.0;
% endfor
    mask_${aidx} *= min(z_slot_mask_${aidx}, 1.0);
% endif
% for v in 'uvw'[:ndims]:
    bc_${v} += control_${aidx} * mask_${aidx} * (${c[f'{v}{aidx}']});
% endfor
% endfor

    ur[0] = ul[0];
% for i, v in enumerate('uvw'[:ndims]):
    ur[${i + 1}] = -ul[${i + 1}] + 2.0*ul[0]*bc_${v};
% endfor
...
<%pyfr:macro name='bc_ldg_state' params='ul, nl, ur' externs='ploc, t, control_params, t_act_interval, actuator_bounds, z_slot_bounds'>
% for v in 'uvw'[:ndims]:
    fpdtype_t bc_${v} = 0.0;
% endfor
